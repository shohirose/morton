...
template <typename Engine, typename T, typename Tag>
...
static void BM_32BitEncodingUsingPreshiftedLookupTable(
  ...
  RunBenchmark<std::mt19937, uint16_t, tag::preshifted_lookup_table>(state);
}
...
static void BM_32BitEncodingUsingBmi(benchmark::State& state) {
  RunBenchmark<std::mt19937, uint16_t, tag::bmi>(state);
}
...
static void BM_64BitEncodingUsingPreshiftedLookupTable(
    benchmark::State& state) {
  RunBenchmark<std::mt19937_64, uint32_t, tag::preshifted_lookup_table>(state);
}
