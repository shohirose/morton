...
template <typename Engine, typename T, int N, typename Tag>
void RunBenchmark(benchmark::State& state) {
  std::random_device seed_gen;
  Engine engine(seed_gen());
...
static void BM_32BitEncodingUsingPreshiftedLookupTable(
  ...
  RunBenchmark<std::mt19937, uint16_t, 10, tag::preshifted_lookup_table>(state);
}
...
static void BM_32BitEncodingUsingBmi(benchmark::State& state) {
  RunBenchmark<std::mt19937, uint16_t, 10, tag::bmi>(state);
}
...
static void BM_64BitEncodingUsingPreshiftedLookupTable(
    benchmark::State& state) {
  RunBenchmark<std::mt19937_64, uint32_t, 21, tag::preshifted_lookup_table>(state);
}
